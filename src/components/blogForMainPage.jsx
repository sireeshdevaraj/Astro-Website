      ...
      <div className="tw-py-8 mobile:tw-py-8 tw-mt-3" id="blogForMainPage" style={{visibility: "hidden"}}>
        <div className="tw-mx-auto tw-max-w-7xl">
          <div className="tw-mx-auto tw-max-w-2xl desktop:tw-mx-0">
            ...
            <p className="tw-mt-2 tw-text-lg tw-leading-8 tw-text-gray-600 dark:tw-text-darkPara">
            ...
            {posts.map((post) => (
              <article key={post.id} className="tw-flex tw-max-w-xl tw-flex-col tw-items-start">
                <div className="tw-flex tw-items-center tw-gap-x-4 tw-text-xs">
                  <time dateTime={post.data.pubDate.toLocaleDateString('en-us', {year: 'numeric',month: 'short',day: 'numeric'})} className="text-gray-500 dark:tw-text-darkPara2">
                    {post.data.pubDate.toLocaleDateString('en-us', {year: 'numeric',month: 'short',day: 'numeric'})}
                  </time>
                  <a
                    // href={post.data?.category || "random"}
                    className="tw-cursor-default tw-text-black tw-relative tw-rounded-full tw-bg-gray-50 dark:tw-bg-gray-600 dark:tw-text-darkPara dark:hover:tw-bg-white dark:hover:tw-text-black tw-px-3 tw-py-1.5 tw-font-medium tw-text-gray-600 hover:tw-bg-gray-100"
                  >
                ...
                <div className="tw-group tw-relative">
                  <h3 className="tw-mt-3 tw-text-lg tw-font-semibold tw-leading-6 tw-text-gray-900 group-hover:tw-text-gray-600 tw-h-7">
                    <a href={`/blog/${post.slug}/`} className="tw-text-black tw-text-black hover:tw-text-gray-600 tw-no-underline dark:tw-text-darkHeading">
                      <span className="tw-absolute tw-inset-0" />
                      {post.data.title}
                    </a>
                  </h3>
                  <p className="tw-mt-5 tw-line-clamp-3 tw-text-sm tw-leading-6 tw-text-gray-600 dark:tw-text-darkPara">{post.data.description}</p>
                </div>
              </article>
            ))}
          </div>
